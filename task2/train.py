...
if __name__ == "__main__":
    ...
    for lang_model_name in lang_model_names:
        for fine_tune in fine_tunes:
            
            lang_model, tokenizer, lm_emb_size = parse_lang_model(lang_model_name)
            vocab_size = len(tokenizer)
        
            device = torch.device(args.device)
        
            train_loader = process_data(args.train_data, tokenizer, device, lang_model_name, batch_size=args.batch_size)
            dev_loader = process_data(args.dev_data, tokenizer, device, lang_model_name, is_dev=True, batch_size=args.batch_size)
            ...
            print(model)
            print("Using model: {}".format(lang_model_name))
            print("Using device: {}".format(device))
            print("Using fine-tuning: {}".format(fine_tune))
            print()
        
            optimizer = torch.optim.Adam(model.parameters(), lr=1e-5)
            criterion = CRF(num_classes, batch_first=True).to(device)

            epochs = 7 if fine_tune else 20
            best_macro_f1 = 0
            ...
            for epoch in range(epochs):
                ...
                if macro_f1 > best_macro_f1:
                    ...
                    torch.save(model, "models/{}_{}.pt".format(lang_model_name, "finetune" if fine_tune else "frozen"))
                    with open("models/{}_{}.txt".format(lang_model_name, "finetune" if fine_tune else "frozen"), "w") as file:
